...
void genImplicitVals(double* arr, double xl, double xr, double yd, double yu, Graph* g, float at) {
	g->math->eval(arr, xl, yd, xr-xl, yu-yd, at - g->startTime, at);
}
...
int main() {
	...
	panel = new GLPanel({ 390, 10, 600, 600 }, { 600, 600 }, layout,
		...
		for (int i = 0; i < graphs.size(); i++) {
			...
			if (graphs[i]->implicit) {
				genImplicitVals(graphs[i]->vals, g_left, g_right, g_down, g_up, graphs[i], glfwGetTime());
	...
	auto setGraph = [&](std::string eq, int index)->void {
		...
		if (result == NULL) {
			graphs[index]->math = new CLMath(graphs[index]->eq[0]->e, g_clInfo, g_windowWidth, g_windowHeight);
		}
